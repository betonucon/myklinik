...
class AsuransiController extends Controller
{
    ...
    public function view(request $request)
    {
        ...
        if(Auth::user()->role_id==1){
            return view('asuransi.view',compact('template','data','id'));
        }else{
            return view('error');
        }
    ...
    {
        error_reporting(0);
        $data = Asuransi::whereIn('active',array(1,0))->orderBy('nama_Asuransi','Asc')->get();
    ...
    public function delete_data(request $request){
        if(Auth::user()->role_id==1){
            $id=decoder($request->id);
            $data = Asuransi::where('id',$id)->update(['active'=>2]);
        }
    ...
    public function switch_status(request $request){
        if(Auth::user()->role_id==1){
            $data = Asuransi::where('id',$request->id)->update(['active'=>$request->act]);
        }
    ...
    public function store(request $request){
        ...
        $rules = [];
        $messages = [];
        $rules['nama_asuransi']= 'required';
        $messages['nama_asuransi.required']= 'Masukan nama Asuransi';
        ...
        if ($validator->fails()) {
            ...
            if($request->id==0){
                    ...
                    $data=Asuransi::where('id',$request->id)->update([
                        'nama_asuransi'=>$request->nama_asuransi,
